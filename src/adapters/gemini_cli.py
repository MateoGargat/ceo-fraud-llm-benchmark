...
class GeminiCLIAdapter(BaseAdapter):
    ...
    async def call(self, system_prompt: str, messages: list[dict[str, str]], temperature: float = 0.7) -> AdapterResponse:
        ...
        try:
            ...
            raise AdapterError("CLI 'gemini' not found in PATH") from exc
        stdout, stderr = await proc.communicate()
        if proc.returncode != 0:
            raise AdapterError(f"CLI 'gemini' failed (exit {proc.returncode}): {stderr.decode('utf-8', errors='replace')}")
